...
namespace Controller
{
    public class Propertity
    {
        ...
        #endregion
        //Menu
        public List<ESHOP_CATEGORy> Loadmenu(int position, int limit, int rank, int showfooter)
        {
            try
            {
                var list = db.ESHOP_CATEGORies.Where(n => n.CAT_STATUS == 1 && (n.CAT_POSITION == position || position == -1) && n.CAT_RANK == rank && (n.CAT_SHOWFOOTER == 1 || showfooter == -1)).OrderByDescending(n => n.CAT_ORDER).Take(limit).ToList();
                return list;
            }
            catch (Exception)
            {
        ...
        public string getnameEshop_pro(int pro_Id)
        {
            try
            {
                var item = db.ESHOP_PROPERTies.Single(n => n.PROP_ID == pro_Id);
                if (item != null)
        ...
        public List<ESHOP_ORDER_ITEM> Load_OrderItems(int m_order_id)
        {
            var AllList = (from o in db.ESHOP_ORDER_ITEMs
                           ...
                           //    o.ESHOP_NEW.NEWS_IMAGE3,
                           //    o.ESHOP_NEW.NEWS_URL,
        ...
        public int get_countOrder(int m_order_id)
        {
            var AllList = (from o in db.ESHOP_ORDER_ITEMs
                           where o.ORDER_ID == m_order_id
                           select new
        ...
        public string get_OrderCode(int m_order_id)
        {
            ...
            if (AllList.Count > 0)
                return AllList[0].ORDER_CODE;
            else
                return "";
        }
        public List<ESHOP_ORDER> get_ListOrder(int m_order_id)
        {
            var AllList = (from o in db.ESHOP_ORDERs
                           where o.ORDER_ID == m_order_id
                           select o).ToList();
